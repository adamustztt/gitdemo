...
class FabWarehouse extends AbstractWarehouse
{
	protected $baseUrl = "";
	protected $token = "";
	protected $account = "";
	...
	protected $userOrder;
	protected $channel = WarehouseChannelEnum::FAB;
	...
	{
		$this->account = config("warehouse.fab.account");
		$this->password = config("warehouse.fab.password");
		$this->secret = config("warehouse.fab.secret");
		$this->baseUrl = config("warehouse.fab.domain");
	...
	private function getExpressInfo($storeId, $expressStatus = "",$order_consignee_id=0)
	{
		...
		$response = $this->sendRequest("post", "/newApi/bus/expressSheet/list", $params);
		...
		QiWeiTool::sendMessageToBaoJing(env("POLICE_FROM")."发B仓库获取面单失败".json_encode($policy_msg,JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT));
		$err = isset($errorMsg[$expressStatus])?"发B仓库ID:$storeId ".$errorMsg[$expressStatus]."不存在":"发B仓库面单类型不存在";
	...
	private function getExpressPrice($expressSheetId)
	{
		...
		$response = $this->sendRequest("post", "/newApi/bus/vipLevel/getExpressFree", $params);
	...
	{
		$response = $this->sendRequest("post", "/newApi/bus/goodsInfo/public/getStore");
		$expressList = $this->handleResponse($response);
		$list = [];
		foreach ($expressList as $k => $v) {
...
//			$expressPrice = $this->getExpressPrice($expressInfo["id"]);
	...
	public function requestProduct($page = 1, $page_size = 100)
	{
		...
		$response = $this->sendRequest("post", "/newApi/bus/goodsInfo/public/list", $params);
		$list=[];
	...
	public function requestOrder($product, $userOrder, $orderConsignee)
	{
		...
		$instance->info("发b数据的快递状态:".$this->expressStatus[$userOrder->source]);
		...
		$response = $this->sendRequest("post", "/newApi/bus/order/sendCargo", $params);
	...
	public function requestOrderQuery($orderConsignee)
	{
		...
		$response = $this->sendRequest("post", "/newApi/bus/pkInfo/list", $params);
		...
		if (empty($data["list"][0]) || empty($data["list"][0]["expressNo"])) {
			...
			if($data["list"][0]["status"] == -1 || $data["list"][0]["status"] ==4) {
				...
				QiWeiTool::sendMessageToBaoJing(env("POLICE_FROM")."FaB仓库".json_encode($policy_msg,JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT),env("POLICE_CODE"));
			}
			return [
	...
	private function handleResponse(ResponseInterface $response)
	{
		...
		switch ($this->requestUrl){
			case $this->baseUrl . "/newApi/bus/goodsInfo/public/getStore": $function="获取仓库"; break;
			case $this->baseUrl . "/newApi/bus/goodsInfo/public/list": $function="获取商品"; break;
			case $this->baseUrl . "/newApi/bus/vipLevel/getExpressFree": $function="获取快递价格"; break;
			case $this->baseUrl . "/newApi/bus/order/sendCargo": $function="请求下单"; break;
			case $this->baseUrl . "/newApi/bus/pkInfo/list": $function="查询订单"; break;
			case $this->baseUrl . "/newApi/bus/info/public/apiAuthority": $function="登录"; break;
		}
		$policy_msg = [
			'功能'=>$function,
			'请求链接'=> $this->requestUrl,
...
//				$add_push = OrderConsigneePushDownService::addPush($this->package_id,1);// 如果是api用户 添加推送信息
	...
	public function login()
	{
		...
		$response = $this->sendRequest("post", "/newApi/bus/info/public/apiAuthority", $params);
